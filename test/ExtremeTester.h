...
public:
    ...
    void setParamValueState(std::vector<TestComposite::Param>& testSignal, const std::vector< std::pair<float, float>>* signalLimits)
    {
        for (int i = (int) state.size() - 1; i >= 0; --i) {
            float min = -10.f;
            float max = 10.f;
        ...
        const char * testName)
    {
        ...
        for (inputState.reset(numInputs); !inputState.isDone(); inputState.next()) {
            ...
            for (paramsState.reset(numParams); !paramsState.isDone(); paramsState.next()) {
                paramsState.setParamValueState(dut.params, &paramLimits);
                for (int i = 0; i < 100; ++i) {
                    dut.step();
